...
class Board(Frame):
    def __init__(self, parent, game, mode):
        ...
        if mode == 0:
            ...
            self.visszvag = Button(self, command=self.game.communicator.i_propose_rematch, text='Rematch',
    ...
    def step(self, r, c, player):
        self.field_buttons[r][c].configure(text=MARKS[player], disabledforeground=COLORS[player], bg='white',
                                           state=DISABLED)

    def undo(self, r, c):
        self.field_buttons[r][c].configure(text='', disabledforeground=COLORS[self.game.player], bg='powder blue',
                                           state='normal')
